...
bsln_data <- read_csv(here::here("Data", "20210331_OSEAR Baseline Data.csv"))
...
f_up_data <- read_csv(here::here("Data", "20210402_APICS01 OSEAR 6mo Data.csv"))
...
# BSLN Missings
    ...
    setdiff(pt_list, bsln_list)                                 # no missings
...
# 6MO Missings 
    ...
    setdiff(pt_list, fup_list)                                  # Oof. 1 BIDMC, 1 IMC, 1 Vanderbilt, 12 JHH APICS01 / OSEAR Only, 8 JHH APICS-C. Total missing 23
    
    # Let's look into where we lost 6mo pts  
        test <- f_up_data %>% 
          select(p1_subid, 
                 str_subset(colnames(f_up_data), "eqol_"), 
                 str_subset(colnames(f_up_data), "adl"))              # 157 pts with Access data entries 
        
        test2 <- test %>% 
          filter(p4_eqol_notdone == 1)                                            # 142 pts with EQOL marked "done" 
        
        test3 <- test2 %>% 
          filter(!is.na(p6_eqol_health))                                          # 129 pts with completed EQ-VAS 
        
        test4 <- test %>% 
          filter(is.na(p6_eqol_health))                                           # HUH. Note that several pts with no EQ-VAS data have I/ADL data 
...
rm(pt_list, bsln_list, bsln_dup, fup_list, f_up_data)
...
# Counts 
    sum(!is.na(eqvas_data$eqvas_dc))        # 180 baseline 
    sum(!is.na(eqvas_data$eqvas_6mo))       # 129 complete @ 6mo
    sum(is.na(eqvas_data$eqvas_6mo))        # 51 missing @ 6mo
...
# Counts of changes
    ...
    sum(!is.na(change_data$diff))                 # 129 complete @ 6mo
    nrow(change_data[change_data$diff > 0,])      # 26 perceived > expected 
    nrow(change_data[change_data$diff == 0,])     # 26 perceived = expected
    nrow(change_data[change_data$diff < 0,])      # 77 perceived < expected
    
    nrow(change_data[change_data$diff >= 8, ])    # 16 perceived > expected by 8 points or more
    nrow(change_data[change_data$diff <= -8,])    # 67 perceived < expected by 8 points or more
    
    nrow(change_data[change_data$diff >= 10, ])   # 16 perceived > expected by 10 points or more
    nrow(change_data[change_data$diff <= -10,])   # 66 perceived < expected by 10 points or more
    
    nrow(change_data[change_data$diff >= 20, ])   #  6 perceived > expected by 20 points or more
    nrow(change_data[change_data$diff <= -20,])   # 40 perceived < expected by 20 points or more
